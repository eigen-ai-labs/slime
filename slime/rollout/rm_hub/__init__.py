...
from .python_reward_adapter import call_reward_function, is_openai_grader_signature
...
async def async_rm(args, sample: Sample, **kwargs):
    if args.custom_rm_path is not None:
        ...
        # Check if using OpenAI grader signature: grade(sample, item)
        if is_openai_grader_signature(rm_function):
...
async def batched_async_rm(
    ...
    **kwargs,
    ...
    if args.custom_rm_path is not None:
        ...
        # Check if using OpenAI grader signature: grade(sample, item)
        if is_openai_grader_signature(rm_function):
            # For OpenAI grader format, call once with all samples as trajectory
